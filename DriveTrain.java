...
public class DriveTrain {
    ...
    final double maxMotorSpeedEncoders = 0.2;
    ...
    public void driveTrainByControls() {
        ...
        if(Constants.stick.getRawButtonPressed(8)){ // determine how many values to switch between we want.
            if(driveSpeedCoefficient == 0.2){ // probably just a slow and a normal is fine. Sometimes too fast is uncomfortable
                driveSpeedCoefficient = 0.5;
            } else if(driveSpeedCoefficient == 0.5){
                driveSpeedCoefficient = 0.2;
            }
        }

        if(Constants.stick.getRawAxis(3) < 0.00){ // doesn't affect autonomous. intake is always front there with driveByInches
        ...
        if (isDriverControlEnabled) {
            ...
            if(direction){
                mecanumDrive.driveCartesian(driveSpeedCoefficient * ySpeed * ySpeed * yDirectionMaintainer, driveSpeedCoefficient * -xSpeed * xSpeed * xDirectionMaintainer,
                driveSpeedCoefficient * zSpeed * zSpeed * zDirectionMaintainer); // shooter is front
            } else {
                mecanumDrive.driveCartesian(driveSpeedCoefficient * -ySpeed * ySpeed * yDirectionMaintainer, driveSpeedCoefficient * xSpeed * xSpeed * xDirectionMaintainer,
                    driveSpeedCoefficient * zSpeed * zSpeed * zDirectionMaintainer); // intake is front
            }
